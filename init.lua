...
xplr.fn.custom.local_help = {
	...
	layout = function(ctx)
		...
		local tablesize = math.min(ctx.layout_size.height - (#d + 2) - 1 - 2 - 1, #filtered)
		if dat.focus < dat.scrolltop + 3 then
			dat.scrolltop = math.max(dat.focus - 3, 1)
		elseif dat.focus - tablesize > dat.scrolltop - 3 then
			dat.scrolltop = math.min(dat.focus - tablesize + 3, #filtered - tablesize + 1)
		end
		...
		for i = dat.scrolltop, math.min(dat.scrolltop + tablesize, #filtered) do
			...
			if v == nil then
				table.insert(rows,
					{ tostring(i) .. " " .. tostring(j) .. " " .. tostring(#filtered), " " .. tostring(#t) })
			else
				local row = ""
				row = row .. v.keys
...
local function setup(args)
	...
	input_buffer_height = args.input_buffer_height or 3
end
